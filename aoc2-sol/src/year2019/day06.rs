...
fn find_res(orbit_map: OrbitData, second_star: bool) -> Result<usize> {
    ...
    for (parent, children) in &orbit_map {
        let _p_idx = idx_map.entry(&parent[..]).or_insert(orbital_graph.add_node(&parent[..]));
        for child in children {
            let _c_idx = idx_map.entry(&child[..]).or_insert(orbital_graph.add_node(&child[..]));
        }
    }
    ...
    if second_star {
        ...
        let you_parent = orbital_graph.neighbors_directed(*you_idx, Incoming).collect::<Vec<_>>();
        let san_parent = orbital_graph.neighbors_directed(*san_idx, Incoming).collect::<Vec<_>>();
        let you_to_san = dijkstra(&orbital_graph, you_parent[0], Some(san_parent[0]), |_| 1);
        let cost = you_to_san.get(&san_parent[0]).ok_or_else(|| anyhow!("no cost for you"))?;
