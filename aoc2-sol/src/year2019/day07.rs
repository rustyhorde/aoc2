...
impl Intcode {
    fn new(intcodes: Vec<I256>) -> (Sender<I256>, Self) {
    ...
    fn parse_op_code(&self, op_code_idx: usize) -> Result<((u8, u8, u8), usize)> {
        let op_code_val = self.intcodes[op_code_idx];
    ...
    fn handle_add(&mut self, idx: &mut usize, modes: (u8, u8, u8)) -> Result<()> {
        let idx_1 = self.intcodes[*idx + 1];
        let idx_2 = self.intcodes[*idx + 2];
        let idx_3 = self.intcodes[*idx + 3];

        let addend_1 = if modes.0 == 0 {
            self.intcodes[to_u(idx_1)?]
        } else {
            idx_1
        };
        let addend_2 = if modes.1 == 0 {
            self.intcodes[to_u(idx_2)?]
        } else {
            idx_2
        };
        {
            self.intcodes[to_u(idx_3)?] = addend_1 + addend_2;
        }
        *idx += 4;
        Ok(())
    }
    ...
    fn handle_mult(&mut self, idx: &mut usize, modes: (u8, u8, u8)) -> Result<()> {
        ...
        self.intcodes[to_u(idx_3)?] = factor_1 * factor_2;
        *idx += 4;
        Ok(())
    }
    ...
    fn handle_input(&mut self, idx: &mut usize, input: I256) -> Result<()> {
        let idx_1 = self.intcodes[*idx + 1];
        self.intcodes[to_u(idx_1)?] = input;
        *idx += 2;
        Ok(())
    }
    ...
    fn handle_jump_if_true(&mut self, idx: &mut usize, modes: (u8, u8, u8)) -> Result<()> {
        ...
        if check_val == I256::ZERO {
            *idx += 3;
            ...
            let idx_2 = to_u(self.intcodes[*idx + 2])?;
            *idx = if modes.1 == 0 {
                to_u(self.intcodes[idx_2])?
            } else {
                idx_2
            };
        }
    ...
    fn handle_less_than(&mut self, idx: &mut usize, modes: (u8, u8, u8)) -> Result<()> {
        let idx_1 = self.intcodes[*idx + 1];
        let idx_2 = self.intcodes[*idx + 2];
        let idx_3 = to_u(self.intcodes[*idx + 3])?;
        ...
        if val_1 < val_2 {
            self.intcodes[idx_3] = I256::ONE;
        } else {
            self.intcodes[idx_3] = I256::ZERO;
        }
        *idx += 4;
        Ok(())
    }
